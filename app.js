...
function renderPerson(p) {
  ...
  let h = `<div class="ph">
    ...
    <div class="pi">
      <div class="pi-rfc">RFC: ${esc(p.rfc)}</div>
      <div class="pi-nom">${esc(p.nombre || '(Sin nombre registrado)')}</div>
      <div class="pi-chips">${Object.keys(p.fuentes).map(s => `<span class="chip">${esc(srcDisplayName(s))}</span>`).join('')}</div>
    </div>
    <div class="tot">
      <div class="tot-n">${totalR}</div>
      <div class="tot-l">registros totales</div>
      <button class="btn" onclick="genPDF()" style="margin-top:4px">⬇ Descargar PDF</button>
    </div>
  </div>`;
        ...
        recs.forEach((rec, ri) => {
          ...
          MESES.forEach(mes => {
            ...
            h += `<div class="mes-card ${isEmpty ? 'empty-mes' : ''}">
              ...
              ${isEmpty ? '<div class="mes-val">—</div>' : `<div class="day-chips">${chips}</div>`}
            </div>`;
          });
          h += `</div>`;
        });
      } else {
        // Tabla genérica — todas las columnas
        const allKeys = [];
        recs.forEach(r => Object.keys(r).forEach(k => { if (!allKeys.includes(k)) allKeys.push(k); }));
        h += `<div class="tw"><table><thead><tr><th>#</th>${allKeys.map(k => `<th>${esc(k)}</th>`).join('')}</tr></thead><tbody>`;
        recs.forEach((rec, ri) => {
          h += `<tr><td class="rn">${ri + 1}</td>${allKeys.map(k => {
            const raw = rec[k];
...
const PDF = {
  ...
  hBar(doc, { x, y, w, h, title, data, total, color, limit=5, labelW=55 }) {
    ...
    items.forEach(([name, val]) => {
      ...
      this.text(doc, `${val} · ${pct.toFixed(0)}%`, x+w-2, cy+3.5, { size:6, color:this.muted, align:'right' });
      cy += rowH;
    });
  },
...
/* ═══════════════════════════════════════════════════════════
   PDF GLOBAL MENSUAL
═══════════════════════════════════════════════════════════ */
async function genReporteGlobalFacMes(mesIndex = new Date().getMonth(), anio = new Date().getFullYear()) {
  if (!DB) { alert('Espera a que cargue la base de datos.'); return; }
  const { jsPDF } = window.jspdf;
  const doc       = new jsPDF('landscape','mm','a4');
  const generated = new Date().toLocaleString('es-MX');
  const mesNombre = MESES_FAC[mesIndex];
  const anioStr   = String(anio);

  const detalle=[], porPersona=new Map(), porServicio=new Map(), porTurno=new Map();
  Object.values(DB).forEach(persona => {
    Object.entries(persona.fuentes||{}).forEach(([src,sheets]) => {
      if (!isSrcFac(src)) return;
      Object.values(sheets||{}).forEach(recs => (recs||[]).forEach(row => {
        const campo = Object.keys(row||{}).find(k => { const nk=norm(k); return nk.includes('FACILIDADES ADMINISTRATIVAS') && nk.includes(mesNombre) && nk.includes(anioStr); });
        if (!campo) return;
        const dias = expandFac(row[campo]); if (!dias.length) return;
        const servicio = String(valNames(row,['SERVICIO','ÁREA','AREA'])||'—').trim()||'—';
        ...
        dias.forEach(dia => {
          detalle.push([persona.rfc||'—', persona.nombre||'SIN NOMBRE', servicio, turno, entrada, salida, dia]);
          const pk = `${persona.rfc} — ${persona.nombre}`;
          porPersona.set(pk,(porPersona.get(pk)||0)+1);
  ...
  const servicios= porServicio.size;
  const turnos   = porTurno.size;
  const arrS = Array.from(porServicio.entries()).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'es'));
  const arrP = Array.from(porPersona.entries()).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'es'));
  const arrT = Array.from(porTurno.entries()).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'es'));

  PDF.header(doc, `Reporte global de facilidades · ${mesNombre} ${anioStr}`, 'Hospital de la Mujer · Resumen mensual', generated);
  PDF.kpi(doc, 14, 40, 56, 'FACILIDADES', total,                             'Eventos',       PDF.amber);
  PDF.kpi(doc, 75, 40, 56, 'PERSONAS',    personas,                          'RFC únicos',    PDF.navy);
  PDF.kpi(doc,136, 40, 56, 'ÁREAS',       servicios,                         'Servicios',     PDF.teal);
  PDF.kpi(doc,197, 40, 55, 'PROM.',       personas?(total/personas).toFixed(1):'0','Por persona', PDF.slate);

  PDF.hBar(doc,{x:14,  y:76, w:136, h:58, title:'Top áreas / servicios',    data:arrS, total, color:PDF.teal,  limit:5, labelW:60});
  PDF.hBar(doc,{x:158, y:76, w:129, h:58, title:'Personas con más facilidades', data:arrP, total, color:PDF.navy,  limit:5, labelW:74});
  PDF.hBar(doc,{x:14, y:142, w:130, h:44, title:'Distribución por turno',   data:arrT, total, color:PDF.slate, limit:4, labelW:44});

  PDF.fill(doc, PDF.soft); PDF.draw(doc, PDF.border);
  doc.roundedRect(152,142,135,44,4,4,'FD');
  PDF.text(doc,'Lectura rápida',158,150,{size:8.5, bold:true});
  PDF.wrap(doc, `Mes: ${mesNombre} ${anioStr}. Total: ${total}. Área con mayor carga: ${arrS[0]?.[0]||'—'} (${arrS[0]?.[1]||0}).`, 158, 158, 122, {size:7});

  if (!detalle.length) PDF.noData(doc, 194, 'No hay facilidades registradas para este mes');
  else {
    doc.addPage('landscape');
    PDF.header(doc,'Detalle global de facilidades','Hospital de la Mujer · Registros del mes seleccionado',generated);
    PDF.table(doc,40,['#','RFC','Nombre','Área / servicio','Turno','Entrada','Salida','Día'],detalle.map((r,i)=>[i+1,...r]),{generated,pageTitle:'Detalle global',pageSub:'Continuación',fontSize:6.5,headColor:PDF.amber,columnStyles:{0:{cellWidth:10,halign:'center'},1:{cellWidth:26},2:{cellWidth:62},3:{cellWidth:58},4:{cellWidth:24,halign:'center'},5:{cellWidth:18,halign:'center'},6:{cellWidth:18,halign:'center'},7:{cellWidth:50}},margin:{left:14,right:14,bottom:26,top:42}});
  }

  PDF.footer(doc, generated);
  doc.save(`REPORTE_GLOBAL_FACILIDADES_${mesNombre}_${anioStr}.pdf`);
}

/* ═══════════════════════════════════════════════════════════
   REPORTES AVANZADOS (UI + PDF + CSV)
═══════════════════════════════════════════════════════════ */
...
function openReporteFacilidadesAvanzado() {
  if (!DB) { alert('Espera a que cargue la base de datos.'); return; }
  const rp = document.getElementById('rp');
  document.getElementById('em').style.display = 'none';
  rp.classList.add('on');
  ...
  const anios    = [...new Set(eventos.map(e=>e.anio))].sort().map(y=>({value:y,label:y}));
  const servicios= [...new Set(eventos.map(e=>e.servicio))].sort((a,b)=>a.localeCompare(b,'es')).map(x=>({value:x,label:x}));
  const turnos   = [...new Set(eventos.map(e=>e.turno))].sort((a,b)=>a.localeCompare(b,'es')).map(x=>({value:x,label:x}));

  rp.innerHTML = `<div class="adv-wrap">
    <div class="adv-head">
      <div class="adv-title">
        <h2>Reportes avanzados · Facilidades administrativas</h2>
        <p>Filtra por persona, mes, año, servicio o turno. Conteo correcto: <b>12/15/05/2026 = 2 facilidades</b>, <b>12-15/05/2026 = 1 facilidad</b>.</p>
      </div>
      <div class="adv-actions">
        <button class="btn" onclick="genPDFAvanzado()">⬇ PDF filtrado</button>
    ...
    <div class="adv-filters">
      ...
      <div class="adv-field"><label>Buscar</label><input id="advQ" placeholder="RFC, nombre o servicio…"></div>
    </div>
    <div id="advResult"></div>
  </div>`;

  fillSelect('advPersona', personas, 'Todas las personas');
  fillSelect('advAnio',    anios,    'Todos los años');
  fillSelect('advServicio',servicios,'Todas las áreas');
  fillSelect('advTurno',   turnos,   'Todos los turnos');
...
function renderAdvanzado() {
  ...
  function barList(title, data, t, limit=12) {
    const shown = data.slice(0,limit);
    if (!shown.length) return `<div class="adv-card"><div class="adv-card-h"><h3>${title}</h3><span>0</span></div><div class="empty-adv">Sin datos</div></div>`;
    return `<div class="adv-card"><div class="adv-card-h"><h3>${esc(title)}</h3><span>${shown.length} conceptos</span></div><div class="bar-list">`+
      shown.map(([name,count])=>{
        const p = pct(count,t);
        return `<div class="bar-row" title="${esc(name)}: ${count} (${p.toFixed(1)}%)">
          <div class="bar-name">${esc(name)}</div>
          <div class="bar-track"><div class="bar-fill" style="width:${Math.max(p,2)}%"></div></div>
          <div class="bar-val">${count} · ${p.toFixed(1)}%</div>
        </div>`;
      }).join('')+`</div></div>`;
  }
  ...
  result.innerHTML = `
    <div class="kpi-grid">
      <div class="kpi"><div class="kpi-num">${total}</div><div class="kpi-lbl">Facilidades otorgadas</div><div class="kpi-sub">Eventos reales filtrados</div></div>
      <div class="kpi"><div class="kpi-num">${personas}</div><div class="kpi-lbl">Personas con facilidad</div><div class="kpi-sub">RFC únicos</div></div>
      <div class="kpi"><div class="kpi-num">${servicios}</div><div class="kpi-lbl">Áreas involucradas</div><div class="kpi-sub">Servicios distintos</div></div>
      <div class="kpi"><div class="kpi-num">${personas?(total/personas).toFixed(1):'0'}</div><div class="kpi-lbl">Promedio por persona</div><div class="kpi-sub">Facilidades/persona</div></div>
      <div class="kpi"><div class="kpi-num">${pct(topS[1],total).toFixed(1)}%</div><div class="kpi-lbl">Área con mayor carga</div><div class="kpi-sub">${esc(topS[0])}</div></div>
    </div>
    <div class="adv-grid">
      ${barList('Porcentaje por área / servicio', porServicio, total, 15)}
      ${barList('Distribución por turno', porTurno, total, 10)}
      ${barList('Comportamiento por mes', porMes, total, 18)}
      ${barList('Personas con más facilidades', porPersona, total, 12)}
    </div>
    <div class="adv-card">
      ...
      ${total ? `<div class="adv-table-wrap"><table><thead><tr><th>#</th><th>RFC</th><th>Nombre</th><th>Área</th><th>Turno</th><th>Entrada</th><th>Salida</th><th>Día</th><th>Mes</th><th>Año</th></tr></thead><tbody>`+
        rows.sort((a,b)=>a.nombre.localeCompare(b.nombre,'es')||a.anio.localeCompare(b.anio)||a.mesIndex-b.mesIndex||a.dia.localeCompare(b.dia,'es')).map((r,i)=>`
          <tr><td class="rn">${i+1}</td><td>${esc(r.rfc)}</td><td>${esc(r.nombre)}</td><td>${esc(r.servicio)}</td><td>${esc(r.turno)}</td><td class="tc">${esc(r.entrada)}</td><td class="tc">${esc(r.salida)}</td><td class="dc">${esc(r.dia)}</td><td>${esc(r.mes)}</td><td>${esc(r.anio)}</td></tr>`).join('')+
        `</tbody></table></div>` : `<div class="empty-adv">No hay resultados con los filtros seleccionados.</div>`}
      <div class="adv-note">💡 Para analizar un año completo deja el mes en "Todos". Para una persona, selecciona su RFC.</div>
    </div>`;
}
...
/* ═══════════════════════════════════════════════════════════
   PDF AVANZADO (3 páginas: dashboard, resumen, detalle)
═══════════════════════════════════════════════════════════ */
function genPDFAvanzado() {
  ...
  /* PÁG 1: DASHBOARD */
  PDF.header(doc,'Reporte Avanzado de Facilidades Administrativas','Hospital de la Mujer · Dashboard visual filtrado',generated);
  ...
  doc.addPage('landscape');
  PDF.header(doc,'Resumen tabular',`Filtros: ${filterText()}`,generated);

  if (!rows.length) {
    PDF.noData(doc,45,'No hay resultados con los filtros seleccionados');
  } else {
    PDF.table(doc,42,['Área / servicio','Total','%'],porServicio.map(([k,v])=>[k,v,`${pct(v,total).toFixed(1)}%`]),{generated,tableWidth:130,margin:{left:14,right:155,bottom:26,top:42},fontSize:7,headColor:PDF.teal,columnStyles:{0:{cellWidth:90},1:{cellWidth:16,halign:'center'},2:{cellWidth:24,halign:'center'}}});
  ...
  PDF.footer(doc, generated);
  doc.save(`REPORTE_VISUAL_FACILIDADES_${new Date().toISOString().slice(0,10)}.pdf`);
}
...
function exportCSV() {
  const rows = applyFilters(buildEventos());
  const headers = ['RFC','Nombre','AreaServicio','Turno','Entrada','Salida','Dia','Mes','Anio'];
  const csv = [headers.join(',')].concat(rows.map(r =>
    [r.rfc,r.nombre,r.servicio,r.turno,r.entrada,r.salida,r.dia,r.mes,r.anio].map(v => '"'+String(v).replace(/"/g,'""')+'"').join(',')
  )).join('\n');
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8;'}));
  a.download = `FACILIDADES_${new Date().toISOString().slice(0,10)}.csv`;
